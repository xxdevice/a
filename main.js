    ...
    $.getJSON("weapons.json" , function(data) {
      ...
      for(var i = 0; i < len; i++) {
        ...
        data[i].type.forEach(function(element,i){
          ...
          console.log(affix[wept].type[index]);
          $("#sheat .page").append($('<div class="affix">').append("<table><caption>"+element.series+"</table></caption>").append($('<td>').append('<label><input type="checkbox"><span>射撃力アップ</span></label>')/* アフィクス*/));
        });
      }
    });
  });
